...
def generate_word_context(input_seq):
    output = []
    for i in range(len(input_seq)):
        ...
        output.append((target_word, samples, labels, deficit))

    return output

def generate_instance(instance):
    target_word = instance[0]
    samples = instance[1]
    labels = instance[2]
    deficit = instance[3]    
    ...
    return (np.array(target_word), np.array(samples), np.array(labels, dtype=int))



if __name__ == "__main__":

    instances = generate_word_context(input_seq)

    with multiprocessing.Pool() as pool:
        training_data = pool.map(generate_instance, instances)
